(function(){
  ...
  const themeFavorites = () => window.SBLTheme?.getFavorites?.() || [];
  const themeRecent = () => window.SBLTheme?.getRecent?.() || [];
  const themeEscape = v => SBL.pokemon.escapeHtml(String(v ?? ''));
  ...
  function myRoster(){ const team=myLeagueTeam(); return (team && DASH.settings.rosters[team]) || []; }
  ...
  function myPokemonStats(species){
    if(!profile || !myLeagueTeam()) return null;
    ...
    for(const r of Object.values(DASH.replays||{})){
      for(const k in (r.mons||{})){
        ...
        if(teamFor(r.players?.[m.side]) !== myLeagueTeam()) continue;
  ...
  async function loadAuthoritativeFranchiseContext(){
    currentMembership = null;
    currentFranchise = null;
    requestedFranchise = null;
    pendingFranchiseRequest = null;
    const leagueId = String(DASH.leagueId || '').trim();
    const userId = String(session?.user?.id || '').trim();
    if(!leagueId || !userId || !SBL.leagueDb?.getMembership || !SBL.franchiseOwnership?.getUserFranchise) return;
    currentMembership = await SBL.leagueDb.getMembership(leagueId, userId, supabase);
    currentFranchise = await SBL.franchiseOwnership.getUserFranchise(leagueId, userId, supabase);
    ...
    pendingFranchiseRequest = pendingRequest || null;

    if(pendingFranchiseRequest?.franchise_id){
      const {data:pendingFranchise,error:pendingFranchiseError} = await supabase
        .from('franchises')
        .select('id,name,status')
        .eq('id', pendingFranchiseRequest.franchise_id)
        .eq('league_id', leagueId)
        .maybeSingle();
      if(pendingFranchiseError) throw pendingFranchiseError;
      pendingFranchiseRequest.franchise = pendingFranchise || null;
    }

    // Identity boundary: getUserFranchise() must only ever resolve the
    // currently authenticated user's assignment. Never accept a franchise
    // result whose userId does not match the live Supabase session.
    if(currentFranchise && String(currentFranchise.userId || '').trim() !== userId){
      console.error('Authoritative franchise identity mismatch.', {
        sessionUserId: userId,
        assignmentUserId: currentFranchise.userId,
        leagueId
      });
      currentFranchise = null;
    }

    if(currentMembership?.requested_franchise_id){
      const {data,error} = await supabase.from('franchises').select('id,name,status').eq('id', currentMembership.requested_franchise_id).eq('league_id', leagueId).maybeSingle();
      if(error) throw error;
      requestedFranchise = data || null;
    }
  }

  function authoritativeClaimStatus(){
    return String(currentMembership?.status || '').toLowerCase();
  }

  function requestedFranchiseName(){
    return String(requestedFranchise?.name || '').trim();
  }

  async function loadTrades(){
    // The league-selection landing page is intentionally league-neutral. In
  ...
  function tradeAllowance(team, kind='team'){
    ...
    const credits = myLeagueTeam() === team ? Math.max(0, Number(profile?.trade_credits) || 0) : 0;
  ...
  async function refreshAll(){
    ...
    await loadAuthoritativeFranchiseContext();
  ...
  function renderAuthGate(){
    ...
    contentEl.innerHTML = `
      <div class="gate">
        <div class="panel">
          <div class="gate-mark">${themeEscape(SOFTWARE_NAME)}</div>
          <h2>Welcome back</h2>
  ...
  function renderTeamClaim(){
    ...
    const intro = needsUsername
      ...
      : (claimStatus === 'pending' && requestedName ? `Your request for ${SBL.pokemon.escapeHtml(requestedName)} is waiting for commissioner approval.` : 'Choose the franchise you want to claim. A commissioner must approve the request before it becomes your active franchise.');
    ...
    (async ()=>{
      let franchises = [];
      try{
        const {data, error} = await supabase.from('franchises').select('id,name,status').eq('league_id', DASH.leagueId).eq('status','active').order('name',{ascending:true});
        if(error) throw error;
        franchises = data || [];
      }catch(e){
        const fieldsEl = document.getElementById('teamClaimFields');
        if(fieldsEl) fieldsEl.innerHTML = `<div class="note danger">Could not load available franchises: ${SBL.pokemon.escapeHtml(e.message || String(e))}</div>`;
        return;
      }
            const available = [];
      for(const f of franchises){
        try{
          if(f.id === currentFranchise?.franchiseId || await SBL.franchiseOwnership.isFranchiseAvailable(DASH.leagueId, f.id, supabase)) available.push(f);
        }catch(e){
          console.warn('Could not check franchise availability:', e);
        }
      }

      const fieldsEl = document.getElementById('teamClaimFields');
      if(!fieldsEl) return;
      fieldsEl.innerHTML = available.length ? `
        <label style="margin-top:12px;">Franchise</label>
        <select id="teamPick">
          <option value="">— Select a franchise —</option>
          ${available.map(f=>`<option value="${SBL.pokemon.escapeHtml(f.id)}" ${f.id===currentFranchise?.franchiseId?'selected':''}>${SBL.pokemon.escapeHtml(f.name)}</option>`).join('')}
        </select>` : `
        <div class="note">No active franchises are currently available to claim.</div>`;

      const btn = document.getElementById('claimBtn');
      btn.disabled = available.length === 0;
      btn.onclick = async ()=>{
        const val = document.getElementById('teamPick')?.value.trim() || '';
        if(!val){ msgEl.className='note danger'; msgEl.textContent='Pick a franchise.'; return; }
        btn.disabled = true;
        try{
          await SBL.franchiseOwnership.requestFranchiseClaim(DASH.leagueId, val, supabase);
          await loadProfile();
          render();
        }catch(e){
          msgEl.className='note danger';
  ...
  function renderPendingOrRejected(){
    const rejected = authoritativeClaimStatus() === 'rejected';
    contentEl.innerHTML = `
      <div class="gate">
        <div class="panel">
  ...
  function myTeamRecord(){
    const team=String(myLeagueTeam()||'').trim();
  ...
  function randomTradeSuggestion(){
    ...
    const candidates=[];
    for(const team of teamsList().filter(t=>t!==myLeagueTeam())){
  ...
  async function submitFeedback(){
    ...
    try{
      ...
      DASH.settings.feedback.unshift({
        ...
        submittedBy:profile.username||'League user',
        team:myLeagueTeam()||'',
  ...
  function renderFeedback(){
    ...
    const mine=items.filter(x=>x.submittedBy===profile.username && (!x.team || x.team===myLeagueTeam()));
    contentEl.innerHTML=`
      <section class="panel">
  ...
  function wireNextBattle(){
    ...
    const battle=nextBattleForTeam(myLeagueTeam());
  ...
  function recentBattleRowHtml(x){
    ...
    return `<div class="recent-battle-row"><div class="recent-battle-week">${SBL.pokemon.escapeHtml(x.week)}</div><div><div class="recent-battle-match">${SBL.pokemon.escapeHtml(myLeagueTeam())}<span class="vs">vs</span>${SBL.pokemon.escapeHtml(x.opponent)}</div><div class="recent-battle-meta">${SBL.pokemon.escapeHtml(x.format)} · ${SBL.pokemon.escapeHtml(date)}</div></div><div class="recent-result ${resultClass}">${resultText}</div><button class="ghost small" type="button" data-myteam-match="${SBL.pokemon.escapeHtml(x.id)}">Summary</button></div>`;
  }
  ...
  function membershipForLeague(league){
    const userId = String(session?.user?.id || '').toLowerCase();
    const email = String(session?.user?.email || '').trim().toLowerCase();
    const members = Array.isArray(league?.members) ? league.members : [];
    return members.find(m => {
      if(typeof m === 'string') return String(m).toLowerCase() === userId || String(m).toLowerCase() === email;
      return String(m?.userId || m?.id || '').toLowerCase() === userId || String(m?.email || '').toLowerCase() === email;
    }) || null;
  }

  function myLeagueMembership(){
    const league = Object.values(DASH.leagues || {}).find(l => String(l?.id || '') === String(DASH.leagueId || ''));
    return membershipForLeague(league);
  }

  function myLeagueTeam(){
    return String(currentFranchise?.franchise?.name || '').trim();
  }

  function leagueEntriesForUser(){
    const registry = DASH.leagues || {};
  ...
  function saveSelectedLeague(id){
    try{ localStorage.setItem('sbl_selected_league_id',String(id||'')); }catch(_){ }
  }

  function leagueRoleFor(league){
    const member = membershipForLeague(league);
  ...
  async function saveLeagueRegistry(registry){
    throw new Error(
      'Legacy league registry persistence is disabled. ' +
      'Membership and franchise ownership must use normalized tables.'
    );

    const settings = JSON.parse(JSON.stringify(DASH.settings || {}));
    settings.leagues = registry || {};
    settings.activeLeagueId = DASH.leagueId || '';
    if(DASH.leagueId && SBL.leagueDb?.isAvailable && await SBL.leagueDb.isAvailable(supabase)){
      const active=registry?.[DASH.leagueId];
      if(active?.state) await SBL.leagueDb.saveState(DASH.leagueId,active.state,supabase);
      for(const league of Object.values(registry||{})){
        if(!league?.id) continue;
        const state=league.state||{teamMap:{},settings:{}};
        await SBL.leagueDb.saveState(league.id,state,supabase);
        const defs=state.settings?.franchiseDefinitions||state.settings?.franchises||{};
        await SBL.leagueDb.upsertFranchises(league.id,Object.entries(defs).map(([name,v])=>({name,conference:v?.conference||''})),supabase);
        for(const member of (Array.isArray(league.members)?league.members:[])){
          if(!member?.userId) continue;
          await SBL.leagueDb.setMembership(league.id,member.userId,{role:member.role||'player',status:member.status||'active',franchise:member.franchise||member.team||''},supabase);
        }
      }
      DASH.leagues = registry;
      return;
    }
    await SBL.replays.saveSharedState({teamMap:DASH.teamMap || {}, settings}, supabase);
    DASH.leagues = settings.leagues;
  }

  function openLeagueActionModal(mode){
    let overlay=document.getElementById('leagueActionOverlay');
    if(overlay) overlay.remove();
    overlay=document.createElement('div');
    overlay.className='league-action-overlay';
    overlay.id='leagueActionOverlay';
    const isJoin=mode==='join';
    overlay.innerHTML=`<div class="league-action-modal" role="dialog" aria-modal="true" aria-labelledby="leagueActionTitle">
      <div class="league-action-kicker">${isJoin?'Join a league':'Create a league'}</div>
      <h2 id="leagueActionTitle">${isJoin?'Join an existing league':'Start your own league'}</h2>
      <p class="note">${isJoin?'Enter the league invite code provided by its administrators. Your request will be sent to the league admins for approval.':'Anyone can create a league. You become its first commissioner and decide the league rules, franchises, season format and membership.'}</p>
      ${isJoin?`<label for="leagueJoinCode">Invite code</label><input id="leagueJoinCode" maxlength="8" autocomplete="off" placeholder="e.g. 7KQ2M8PD">`:`<label for="newLeagueName">League name</label><input id="newLeagueName" maxlength="60" placeholder="e.g. SBL Season League"><label for="newLeagueDescription">Description <span class="note">optional</span></label><textarea id="newLeagueDescription" rows="3" maxlength="180" placeholder="What is this league?"></textarea><div class="league-action-info"><span>Membership</span><strong>Invite code required</strong><small>League admins control who joins and what rules apply.</small></div>`}
      <div id="leagueActionMsg" class="note"></div>
      <div class="league-action-buttons"><button class="ghost" id="leagueActionCancel">Cancel</button><button class="primary" id="leagueActionSubmit">${isJoin?'Request to join':'Create league'}</button></div>
    </div>`;
    document.body.appendChild(overlay);
    const close=()=>overlay.remove();
    document.getElementById('leagueActionCancel').onclick=close;
    overlay.addEventListener('click',e=>{if(e.target===overlay)close();});
    const msg=document.getElementById('leagueActionMsg');
    document.getElementById('leagueActionSubmit').onclick=async()=>{
      const btn=document.getElementById('leagueActionSubmit'); btn.disabled=true; msg.className='note'; msg.textContent='';
      try{
        const registry=JSON.parse(JSON.stringify(DASH.leagues||{}));
        if(isJoin){
          const code=document.getElementById('leagueJoinCode').value.trim().toUpperCase();
          if(!/^[A-HJ-NP-Z2-9]{8}$/.test(code)){msg.className='note danger';msg.textContent='Enter the 8-character invite code.';btn.disabled=false;return;}
          let target=Object.values(registry).find(l=>String(l.joinCode||'').toUpperCase()===code);
          if(SBL.leagueDb?.isAvailable && await SBL.leagueDb.isAvailable(supabase)){
            target=await SBL.leagueDb.findByJoinCode(code,supabase);
            if(!target){msg.className='note danger';msg.textContent='No league was found with that invite code.';btn.disabled=false;return;}
            const existing=await SBL.leagueDb.getMembership(target.id,String(session.user.id),supabase);
            if(existing){msg.className='note';msg.textContent=existing.status==='pending'?'Your join request is already pending.':'You are already a member of this league.';btn.disabled=false;return;}
            await SBL.leagueDb.requestJoin(target.id,session.user,supabase);
            msg.className='note ok'; msg.textContent=`Join request sent to ${target.name||'the league'}.`;
            setTimeout(close,700); renderLeagueDashboard();
            return;
          }
          if(!target){msg.className='note danger';msg.textContent='No league was found with that invite code.';btn.disabled=false;return;}
          target.members=Array.isArray(target.members)?target.members:[];
          const uid=String(session.user.id);
          const existing=target.members.find(m=>String(m?.userId||m?.id||'')===uid);
          if(existing){msg.className='note';msg.textContent=existing.status==='pending'?'Your join request is already pending.':'You are already a member of this league.';btn.disabled=false;return;}
          target.members.push({userId:uid,email:session.user.email||'',username:profile?.username||'',franchise:'',team:'',role:'player',status:'pending',requestedAt:new Date().toISOString()});
          ...
          if(!name){msg.className='note danger';msg.textContent='Give the league a name first.';btn.disabled=false;return;}
          if(Object.values(registry).some(l=>String(l.name||'').trim().toLowerCase()===name.toLowerCase())){msg.className='note danger';msg.textContent='A league with that name already exists.';btn.disabled=false;return;}
          let id=name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||'league';
          const base=id; let n=2; while(registry[id]) id=`${base}-${n++}`;
          let joinCode=makeLeagueCode(); while(Object.values(registry).some(l=>String(l.joinCode||'').toUpperCase()===joinCode)) joinCode=makeLeagueCode();
          const state={teamMap:{},settings:{caseInsensitiveNames:true,rosters:{},franchises:{},franchiseDefinitions:{},conferenceNames:{a:'Conference A',b:'Conference B'},leagues:{},activeLeagueId:id,activeSeason:'Season 1',seasonArchives:{},leagueUpdates:[],draft:{},freeAgency:{mons:[]},tradeCredits:{},finals:SBL.finals?.defaultFinalsState?SBL.finals.defaultFinalsState():{}} ,replays:{}};
          if(SBL.leagueDb?.isAvailable && await SBL.leagueDb.isAvailable(supabase)){
          ...
          registry[id]=league;
          DASH.leagues=registry; DASH.leagueId=id; DASH.leagueName=name; saveSelectedLeague(id);
    ...
    setTimeout(()=>document.getElementById(isJoin?'leagueJoinCode':'newLeagueName')?.focus(),0);
  }

  function renderLeagueDashboard(){
    const leagues=leagueEntriesForUser();
    const pendingLeagues=Object.values(DASH.leagues||{}).filter(l=>{
      const m=membershipForLeague(l);
      return String(m?.status||'').toLowerCase()==='pending';
    });
    const selected=leagues.find(l=>String(l.id)===String(DASH.leagueId))||null;
    const signedInAs=session?.user?.email||profile?.username||'your account';
    const esc=v=>SBL.pokemon.escapeHtml(String(v??''));

    if(!leagues.length){
      contentEl.innerHTML=`
        <div class="league-home-empty-state">
          <section class="league-home-intro">
            <span class="league-home-overline">${themeEscape(SOFTWARE_NAME)} / League directory</span>
            <div class="league-home-empty-number">01</div>
            <h2>Nothing here yet.</h2>
            <p>You are not currently a member of a league. Create one and make it yours, or enter an invite code from a league administrator.</p>
          </section>
          <section class="league-home-empty-actions">
            <button type="button" class="league-home-action-primary" id="createLeagueBtn">Create a league <span>↗</span></button>
            <button type="button" class="league-home-action-secondary" id="joinLeagueBtn">Join with an invite code</button>
          </section>
        </div>
        ${pendingLeagues.length?`<section class="league-home-pending"><div class="league-home-pending-head"><span>Pending</span><strong>${pendingLeagues.length} request${pendingLeagues.length===1?'':'s'}</strong></div>${pendingLeagues.map(l=>`<div class="league-home-pending-row"><strong>${esc(l.name||l.id)}</strong><span>Awaiting approval</span></div>`).join('')}</section>`:''}
        <footer class="league-home-footer"><span>Signed in as ${esc(signedInAs)}</span><button type="button" id="leagueAccountBtn">Account</button></footer>`;
    }else{
      contentEl.innerHTML=`
        <div class="league-home-shell">
          <section class="league-home-intro">
            <span class="league-home-overline">${themeEscape(SOFTWARE_NAME)} / League directory</span>
            <div class="league-home-mark" aria-hidden="true">${themeEscape(SOFTWARE_NAME)}</div>
            <h2>Choose your league.</h2>
            <p>Every league is its own competition. Open one to enter its roster, season, draft and match workspace.</p>
            <div class="league-home-intro-meta"><span><b>${leagues.length}</b> active ${leagues.length===1?'league':'leagues'}</span><span>Select one to continue</span></div>
          </section>

          <section class="league-home-directory" aria-label="Your leagues">
            <div class="league-home-directory-head"><div><span>Your leagues</span><strong>Workspace selection</strong></div><span class="league-home-directory-count">${String(leagues.length).padStart(2,'0')}</span></div>
            <div class="league-home-list">
              ${leagues.map((l,i)=>{
                const hasState=!!(l.state && typeof l.state==='object');
                const ls=hasState && l.state.settings && typeof l.state.settings==='object' ? l.state.settings : null;
                const hasFranchiseState=!!ls && (Object.prototype.hasOwnProperty.call(ls,'franchiseDefinitions') || Object.prototype.hasOwnProperty.call(ls,'franchises') || !!l.state.teamMap);
                const count=hasFranchiseState ? new Set([...Object.keys(ls.franchiseDefinitions||{}),...Object.keys(ls.franchises||{}),...Object.values(l.state.teamMap||{})].filter(Boolean)).size : null;
                const seasonLabel=ls && ls.activeSeason ? String(ls.activeSeason) : null;
                const current=selected && String(l.id)===String(selected.id);
                const membership=membershipForLeague(l);
                const pending=String(membership?.status||'').toLowerCase()==='pending';
                const statsHtml=seasonLabel || count!==null ? `<span>${esc(seasonLabel||'Season unavailable')}</span><span>${count===null?'Franchise data unavailable':`${count} franchise${count===1?'':'s'}`}</span>` : '<span>League data unavailable</span>';
                return `<button type="button" class="league-home-row ${current?'selected':''}" data-select-league="${esc(l.id)}">
                  <span class="league-home-row-index">${String(i+1).padStart(2,'0')}</span>
                  <span class="league-home-row-main"><strong>${esc(l.name||l.id)}</strong><span>${esc(l.description||'League workspace')}</span></span>
                  <span class="league-home-row-stats">${statsHtml}</span>
                  <span class="league-home-row-status">${pending?'Pending':current?'Selected':'Open'} <b>↗</b></span>
                </button>`;
              }).join('')}
            </div>
            <div class="league-home-directory-actions">
              <button type="button" class="league-home-action-secondary" id="joinLeagueBtn">Join a league</button>
              <button type="button" class="league-home-action-primary" id="createLeagueBtn">Create a league <span>↗</span></button>
            </div>
          </section>
        </div>
        ${pendingLeagues.length?`<section class="league-home-pending"><div class="league-home-pending-head"><span>Pending requests</span><strong>${pendingLeagues.length}</strong></div>${pendingLeagues.map(l=>`<div class="league-home-pending-row"><strong>${esc(l.name||l.id)}</strong><span>Waiting for league administrators</span></div>`).join('')}</section>`:''}
        <footer class="league-home-footer"><span>Signed in as ${esc(signedInAs)}</span><button type="button" id="leagueAccountBtn">Account</button></footer>`;
    }

    contentEl.querySelectorAll('[data-select-league]').forEach(btn=>btn.addEventListener('click',async()=>{
      const id=btn.dataset.selectLeague;
      const target=leagues.find(l=>String(l.id)===String(id));
      if(!target || (SBL.leagueDb?.isUuid && !SBL.leagueDb.isUuid(id))) return;
      const status=btn.querySelector('.league-home-row-status');
      if(status) status.innerHTML='Opening…';
      btn.disabled=true;
      try{
        saveSelectedLeague(id);
        DASH.leagueId=id;
        DASH.leagueName=target.name||id;
        if(SBL.leagueDb?.isAvailable && await SBL.leagueDb.isAvailable(supabase)){
          const snap=await SBL.leagueDb.getSnapshot(id,supabase);
          if(!snap) throw new Error('That league could not be loaded.');
          const rows=await SBL.leagueDb.loadReplays(id,supabase);
          const {sharedState,replays}=SBL.replays.partition([{replay_id:'__dashboard_state__',replay_data:snap.state},...rows.map(r=>({replay_id:r.replay_id,replay_data:r.replay_data}))]);
          const state=sharedState||{};
          DASH.teamMap=state.teamMap||{};
          DASH.settings=Object.assign({rosters:{},freeAgency:{mons:[]}},state.settings||{});
          DASH.settings.rosters=normalizeStoredRosters(DASH.settings.rosters||{});
        ...
        btn.disabled=false;
        if(status) status.textContent='Could not open';
        const msg=document.createElement('div');
        msg.className='note danger league-open-error';
        msg.textContent=e.message||String(e);
        contentEl.prepend(msg);
      }
    }));
    document.getElementById('leagueAccountBtn')?.addEventListener('click',()=>{ leagueLanding=true; activeTab='account'; document.body.classList.add('league-selection-mode'); render(); });
    document.getElementById('createLeagueBtn')?.addEventListener('click',()=>openLeagueActionModal('create'));
    document.getElementById('joinLeagueBtn')?.addEventListener('click',()=>openLeagueActionModal('join'));
  }

  function renderMyTeam(){
    const pageTitle=document.querySelector('#app > header h1');
    const pageSub=document.querySelector('#app > header .sub');
    if(pageTitle) pageTitle.textContent='My Team';
    if(pageSub) pageSub.textContent=`${DASH.leagueName || 'League'} · Manage your roster, trades and league activity`;
    if(authoritativeClaimStatus() !== 'active' && authoritativeClaimStatus() !== 'approved'){
      ...
      <section class="myteam-hero">
        <div class="myteam-hero-grid">
          <div>
            ...
            <div class="myteam-title">${SBL.pokemon.escapeHtml(myLeagueTeam())}</div>
  ...
  function renderProposeTrade(){
    ...
    const roster = myRoster();
    const otherTeams = teamsList().filter(t=>t!==myLeagueTeam());
    ...
    function updateTradeSummary(){
      ...
      bits.push(`${tradeLimitMessage(myLeagueTeam(), targetIsFA ? 'freeagency' : 'team')}`);
      if(!targetIsFA){
    ...
    document.getElementById('submitTradeBtn').onclick = async ()=>{
      ...
      const proposerAllowance = tradeAllowance(myLeagueTeam(), targetIsFA ? 'freeagency' : 'team');
  ...
  function renderTrades(){
    if(authoritativeClaimStatus() !== 'active'){
  ...
  function markTradeTabSeen(tab){
    if(!myLeagueTeam()) return;
    const seen = getTradeSeen();
    const mine = myLeagueTeam();
  ...
  function renderAccount(){
    ...
    contentEl.innerHTML = `<div class="account-page">
      <div class="account-hero panel">
        ...
        <div class="account-hero-actions"><button type="button" class="ghost" id="accountBackToLeagues">Back to leagues</button><div class="account-avatar"><span>${SBL.pokemon.escapeHtml((profile?.username||'?').slice(0,1).toUpperCase())}</span></div></div>
      </div>
      ...
      <div class="account-layout">
        <section class="panel account-card">
          ...
          <div class="account-profile-list">
            ...
            <div class="account-profile-row"><span>Franchise</span><strong>${leagueLanding ? 'Managed per league' : SBL.pokemon.escapeHtml(profile?.team_name || '—')}</strong></div>
        ...
        <div class="account-theme-picker">
          ...
          <div class="custom-theme-panel" id="customThemePanel" hidden>
            ...
            <div class="custom-theme-grid">
              ${[['name','Name','text'],['bg','Background','color'],['panel','Surface','color'],['panelAlt','Elevated surface','color'],['border','Border','color'],['text','Text','color'],['textDim','Muted text','color'],['accent','Accent','color'],['accentText','Accent text','color']].map(([key,label,type])=>type==='text'?`<label class="custom-field"><span>${label}</span><input id="custom_${key}" type="text" value="${themeEscape(key==='name'?'My Theme':(currentTheme?.[key]||''))}"></label>`:`<label class="custom-color-row"><span>${label}</span><input id="custom_${key}" type="color" value="${themeEscape(currentTheme?.[key]||'#888888')}"><code id="custom_${key}_hex">${themeEscape(currentTheme?.[key]||'#888888')}</code></label>`).join('')}
            </div>
            <div class="foot-actions"><button type="button" class="ghost" id="customThemeCancel">Cancel</button><button type="button" class="primary" id="customThemeSave">Save & apply</button></div>
          </div>
        </div>
      </section>
  ...
  function renderMatches(){
    if(authoritativeClaimStatus()!=='active'){contentEl.innerHTML='<div class="panel"><div class="empty-state">Your team claim needs to be approved before viewing match history.</div></div>';return;}
    const rows=myTeamMatchData();
    contentEl.innerHTML=`<div class="panel"><div class="myteam-section-head"><div><h2>Match History</h2><div class="note">Every processed replay involving ${SBL.pokemon.escapeHtml(myLeagueTeam())}.</div></div></div><div class="recent-battles-list">${rows.length?rows.map(recentBattleRowHtml).join(''):'<div class="empty-state">No match history yet.</div>'}</div></div>`;
  }
  ...
  function render(){
    ...
    // DASH membership snapshot. currentMembership/currentFranchise are loaded
    // from the authenticated user's normalized league records above.
    if(!profile.username){
      document.getElementById('tabs')?.remove();
      renderTeamClaim();
      return;
    ...
    const authoritativeStatus = authoritativeClaimStatus();
    const authoritativeRole = String(currentMembership?.role || '').toLowerCase();
    const hasAuthoritativeFranchise = !!currentFranchise?.franchiseId;
    const canEnterWithoutFranchise = ['commissioner','manager'].includes(authoritativeRole);

    // Case 1: active membership + active assignment. Always enter the normal
    // workspace. This must never fall through to the claim screen.
    if(authoritativeStatus === 'active' && hasAuthoritativeFranchise){
      // continue to workspace rendering below
    }
    // Case 4: commissioners/managers may enter even when they have no
    // franchise. This preserves the existing admin-access rule.
    else if(authoritativeStatus === 'active' && canEnterWithoutFranchise){
      // continue to workspace rendering below
    }
    // Case 2: ordinary active member with no authoritative assignment is a
    // valid member state and must enter the franchise request flow.
    else if(authoritativeStatus === 'active' && !hasAuthoritativeFranchise){
      document.getElementById('tabs')?.remove();
      if(pendingFranchiseRequest) return renderPendingOrRejected();
  ...
  function openRecordGamesPopup(kind){
    ...
    overlay.innerHTML=`<div class="myteam-replay-box" style="max-width:760px"><div class="myteam-summary-header"><div><div class="myteam-summary-kicker">${kind==='win'?'Wins':'Losses'}</div><h3>${kind==='win'?'Games you won':'Games you lost'}</h3><div class="myteam-summary-result">${matches.length} ${kind==='win'?'win':'loss'}${matches.length===1?'':'s'}</div></div><div class="myteam-summary-meta"><span>${SBL.pokemon.escapeHtml(myLeagueTeam())}</span></div></div><div class="recent-battles-list">${matches.length?matches.map(recentBattleRowHtml).join(''):'<div class="empty-state">No recorded games in this category.</div>'}</div><div class="myteam-summary-actions"><button class="ghost" type="button" id="recordGamesClose">Close</button></div></div>`;
    document.body.appendChild(overlay);document.body.style.overflow='hidden';
    const close=()=>{overlay.remove();document.body.style.overflow='';};
